...
with st.expander("📊 Machine Learning - Prediksi Target Operasi"):
    uploaded_file_ml = st.file_uploader("Unggah Data Hasil Pemeriksaan (CSV)", type=["csv"], key="ml")
    if uploaded_file_ml:
        df = pd.read_csv(uploaded_file_ml, delimiter=';')
        st.write("Contoh data:", df.head())

        if 'Hasil Pemeriksaan' in df.columns:
            df['label'] = df['Hasil Pemeriksaan'].astype(int)
            fitur_teknikal = [
                'arus_hilang', 'over_current', 'over_voltage', 'v_drop',
                'cos_phi_kecil', 'active_power_negative', 'unbalance_I',
                'active_p_lost', 'arus_kecil_teg_kecil', 'current_loop', 'freeze',
                'In_more_Imax', 'active_power_negative_siang', 'active_power_negative_malam'
            ]
            try:
                X = df[fitur_teknikal].astype(int)
                y = df['label']
                ...
                df['probabilitas'] = model.predict_proba(X)[:, 1]
                st.subheader("Hasil Prediksi Target Operasi:")
                st.dataframe(df[['LOCATION_CODE', 'prediksi', 'probabilitas'] + fitur_teknikal])

                csv = df.to_csv(index=False).encode('utf-8')
                st.download_button("📥 Unduh Hasil Prediksi", data=csv, file_name="hasil_prediksi_TO.csv", mime="text/csv")

            except Exception as e:
                st.error(f"Gagal melatih model: {e}")
        else:
            st.warning("Kolom 'Hasil Pemeriksaan' tidak ditemukan dalam file.")
    else:
        st.info("Silakan unggah file hasil pemeriksaan untuk memulai pelatihan model.")
